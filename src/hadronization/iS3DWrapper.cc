...
void iS3DWrapper::PassHadronListToJetscape()
{
  ...
  for (unsigned int iev = 0; iev < iS3D_.final_particles_.size(); iev++)
    {
      std::vector<shared_ptr<Hadron>> hadrons;
      unsigned int nparticles = ( iS3D_.final_particles_[iev].size() );
      //VERBOSE(4) << "number of particles = " << nparticles;
      for (unsigned int ipart = 0; ipart < nparticles; ipart++)
	{
	  Sampled_Particle current_hadron = iS3D_.final_particles_[iev][ipart];
	  int hadron_label = 0;
	  int hadron_status = -1;
